...
- **Stack**: React, Vue, Next.js, or default to `react`
...
Get implementation-specific best practices. If user doesn't specify a stack, **default to `react`** (프로젝트의 기술 스택에 맞게 조정).
...
**User request:** "대시보드 UI 디자인해줘"

### Step 1: Analyze Requirements
- Product type: SaaS / Dashboard
- Style keywords: modern, clean, dark mode, professional
...
```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "saas dashboard analytics modern clean" --design-system -p "My Project"
```
...
```bash
# Get UX guidelines for animation and real-time interaction
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "animation real-time interaction" --domain ux

# Get dark mode typography options
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "monospace futuristic tech" --domain typography
```

### Step 4: Stack Guidelines

```bash
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "component state animation" --stack react
```

**Then:** Synthesize design system + detailed searches and implement the design with React + TailwindCSS + Framer Motion.
