...
21명의 전문가 에이전트가 디자인, 보안, 테스트, 코드 품질을 처음부터 강제합니다.
국정조사급 버그 조사(5 에이전트 팀) + 대규모 보안 감사(8+3 에이전트 2-pass) + 6개 언어 품질 게이트 + 디자인 레퍼런스 7개 + Anti-Slop 패턴 탐지 + 포인터 아키텍처(단일 진실 공급원)로,
...
│                       wj-magic v4.11.0                          │
│                                                                │
│   8 커맨드 · 16 스킬 · 21 에이전트 · 7 훅 · 7 규칙 · 8 MCP      │
│                                                                │
│  ┌───────────┐  ┌─────────────────┐  ┌────────────────────┐   │
│  │  커맨드    │  │  스킬            │  │  자동 훅            │  │
│  │           │  │                 │  │                    │  │
│  │  init     │  │  investigate    │  │  L1 정적 감사       │  │
│  │  loop     │  │  devrule / tdd  │  │  L2 타입 체크       │  │
...
│  ┌──────────────────────────────────────────────────────────┐ │
│  │  전문 에이전트 팀 (21명)                                   │ │
│  │  ─ 구현 (5)   frontend-dev · backend-dev · engine-dev    │ │
│  │              design-dev · test-engineer                  │ │
│  │  ─ 리뷰 (4)   qa-reviewer · design-reviewer              │ │
│  │              docs-keeper · regression-hunter             │ │
│  │  ─ 조사 (4)   web-researcher · code-analyst              │ │
│  │              perf-analyst · injection-hunter             │ │
│  │  ─ 보안 (8)   security-auditor · auth-auditor            │ │
│  │              api-security-auditor · client-security-…    │ │
│  │              config-auditor · crypto-auditor             │ │
│  │              data-integrity-auditor · supply-chain-…     │ │
│  └──────────────────────────────────────────────────────────┘ │
│                                                                │
│  ┌──────────────────────────────────────────────────────────┐ │
│  │  자동 적용 규칙 (rules/) · 7개                             │ │
...
│                                                                │
│  ┌──────────────────────────────────────────────────────────┐ │
│  │  .mcp.json — 8개 MCP 자동 등록 (플러그인 설치 시 활성화)    │ │
...
- hooks 이벤트 바인딩
- rules glob 매칭 활성화
- `.mcp.json` 8개 MCP 서버 자동 등록 (plugin scope)
- 권한 허용/차단 설정
...
사용자 입력
  ...
  └─ "전수 점검" ────→ /wj-magic:cto-review (스킬) → 코드베이스 전수 점검
    ...
    └──────────────────────────────────────────────────────────┘
          ...
          ▼
    구현 완료 후
      ├→ /wj-magic:commit ──→ 한글 커밋 메시지 자동 생성
      ├→ docs-keeper ──→ 구조 변경 시 문서 동기화
      └→ /wj-magic:learn ───→ 실패·반복 실수 시 교훈 축적
```
...
`/wj-magic:devrule`과 `/wj-magic:loop`이 에이전트를 선택하는 조건:
...
```
/wj-magic:devrule 또는 /wj-magic:loop 실행
  │
  ▼
...
"이거 구현해줘", "버그 고쳐줘" 같은 직접 요청 시 `/wj-magic:devrule` 스킬이 자동 작동합니다.

```
사용자: "로그인 기능 만들어줘"
  │
  ▼
/wj-magic:devrule 자동 트리거
  │
...
│         │  FAIL → 재위임 (최대 2회)           │     │
│         │         + /wj-magic:learn 교훈 축적       │     │
│         ▼                                   │     │
│  E. 완료 처리                                │     │
│     E-1. tasks.json → "done"                │     │
│     E-2. /wj-magic:commit 커밋                     │     │
│     E-3. docs-keeper (구조 변경 시)           │     │
│     E-4. /wj-magic:learn (QA FAIL 있었으면)         │     │
│     E-5. 다음 task 자동 전진 ─────────────────┘     │
│                                             │
└─────────────────────────────────────────────┘
  │
  ▼
모든 task 완료 → /wj-magic:verify (최종 검증)
```
...
│
├── commands/                 ← 슬래시 커맨드 (5개)
│   ├── init.md                  /wj-magic:init — 스캐폴딩
│   ├── loop.md                  /wj-magic:loop — 자율 루프
│   ├── verify.md                /wj-magic:verify — 빌드+테스트
│   ├── check.md                 /wj-magic:check — 품질 전수 점검
│   └── help.md                  /wj-magic:help — 가이드
│
├── skills/                   ← 자동 트리거 스킬 (13개)
│   ├── investigate/skill.md     /wj-magic:investigate — 국정조사급 심층 이슈 분석
│   ├── commit/skill.md          /wj-magic:commit — 한글 커밋
│   ├── devrule/                 /wj-magic:devrule — 구조 적용 개발
│   │   ├── skill.md
│   │   └── references/          MACOS_DEV_REFERENCE, TROUBLESHOOTING
│   ├── tdd/skill.md             /wj-magic:tdd — Red-Green-Refactor TDD 강제
│   ├── design/skill.md          /wj-magic:design — 디자인 기획+구현
│   ├── polish/skill.md          /wj-magic:polish — 디자인 개선
│   ├── brainstorm/skill.md      /wj-magic:brainstorm — 아이디어 → 설계 문서
│   ├── plan/skill.md            /wj-magic:plan — 스펙 → 구현 계획
│   ├── learn/skill.md           /wj-magic:learn — 교훈 축적
...
│   ├── journal.sh               작업 일지 기록
│   └── investigation-utils.sh   /wj-magic:investigate 헬퍼 (git-suspects, bisect, report-init)
│
...
│   └── kotlin/standards.md
│
├── templates/                ← /wj-magic:init 스캐폴딩 템플릿
...
| 커맨드 | 인자 | 역할 |
|--------|------|------|
| `/wj-magic:help` | — | 커맨드 가이드 출력 |
| `/wj-magic:init` | `[--with-prd]` | 클린 스캐폴딩 (docs/ + .dev/ + CLAUDE.md) |
| `/wj-magic:loop` | `plan <요구사항> \| start [task-id] \| stop \| status` | PRD/task 생성 + 세션 내 자율 루프 |
| `/wj-magic:verify` | `[--smoke]` | 전체 빌드+테스트 최종 검증 |
| `/wj-magic:check` | — | 품질 전수 점검 (6개 언어) |

## 스킬 레퍼런스

| 스킬 | 역할 | 트리거 |
|------|------|--------|
| `/wj-magic:investigate` | 국정조사급 심층 이슈 분석 (5 에이전트 팀 + 웹 리서치 + 자동 수정) | "버그", "조사", "안된다", "느려", "보안", "원인 찾아줘" |
| `/wj-magic:devrule` | 프로젝트 구조 적용 개발 (S/M/L 규모별 전략) | "구현해줘", "만들어줘", "고쳐줘" |
| `/wj-magic:tdd` | Red-Green-Refactor TDD 프로세스 강제 | "TDD로", "테스트 먼저" |
| `/wj-magic:brainstorm` | 아이디어 → 설계 문서 1:1 대화 | "기획 도와줘", "아이디어 있어", "어떻게 만들지" |
| `/wj-magic:plan` | 스펙 → 구현 계획 (태스크 분해) | "계획 세워줘", "태스크 나눠줘" |
| `/wj-magic:design` | 디자인 기획 + 구현 (방향 설정 → 구현 → 리뷰) | "디자인해줘", "UI 만들어줘", "랜딩페이지" |
...
`/wj-magic:investigate` 실행 시에는 조사 전문 4명이 병렬 투입됩니다.
...
/wj-magic:investigate Phase 1 병렬 조사팀 (run_in_background: true):
...
Phase 4: 검증 (L1/L2/L3 게이트)
    │
Phase 5: 리포트 + Memory MCP 저장 + /wj-magic:learn
```
...
| `web-researcher` | Context7+WebSearch CVE/유사 이슈 조사 | `/wj-magic:investigate` Phase 1 |
| `code-analyst` | Serena MCP 심볼 추적, SBFL 의심도 분석 | `/wj-magic:investigate` Phase 1 |
| `perf-analyst` | Core Web Vitals, N+1/재렌더링 안티패턴 | `/wj-magic:investigate` Phase 1 |
| `regression-hunter` | git bisect 자동화, blame 회귀 분석 | `/wj-magic:investigate` Phase 1 |
...
전체 빌드/smoke test는 `/wj-magic:verify`로 수동 실행.
...
## MCP 서버 자동 등록 (`.mcp.json`)

플러그인 루트의 `.mcp.json`에 정의된 8개 MCP 서버가 **플러그인 설치 시 자동으로 plugin scope에 등록**됩니다 (Claude Code 공식 plugin 스펙).
모든 구현·리뷰·감사 에이전트는 작업 중 Serena·Context7·Sequential-thinking 3종을 **반드시** 사용하도록 강제됩니다 (각 에이전트 파일 상단 "⛔ MCP 필수 사용" 섹션).

| MCP 서버 | 용도 | 강제 사용처 |
|----------|------|-----------|
| **Serena** | 시맨틱 코드 탐색 (`find_symbol`, `find_referencing_symbols`) | 모든 구현·리뷰·감사 에이전트 (코드 수정 전 필수) |
| **Context7** | 라이브러리 공식 문서 실시간 조회 (`resolve-library-id` → `query-docs`) | 모든 구현·리뷰·감사 에이전트 (라이브러리 API 사용 전 필수) |
| **Sequential Thinking** | 단계별 추론 (`mcp__sequential-thinking__sequentialthinking`) | 모든 에이전트 (복잡한 task 시작 시 필수) |
| **Playwright** | 브라우저 자동화 테스트 | frontend 규칙, perf-analyst |
| **Chrome DevTools** | 브라우저 디버깅, 성능 분석 | frontend 규칙, perf-analyst |
| **shadcn/ui** | UI 컴포넌트 레지스트리 조회 | frontend 규칙, design-dev |
| **Magic (21st.dev)** | AI 컴포넌트 빌더 | design-dev |
| **Memory** | 세션 간 메모리 저장 | learn 스킬 |

> **SubagentStop 훅이 MCP 호출 흔적을 정적 검출**합니다 — 서브에이전트가 위 3종(Serena/Context7/Sequential-thinking) 중 어느 것도 호출하지 않고 종료하면 경고가 출력되어 추측 기반 작업을 사후 식별할 수 있습니다.
