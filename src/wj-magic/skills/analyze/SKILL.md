...
description: >
  ...
  코드를 직접 수정하지 않는다. 수정은 /wj-magic:devrule로 이어간다.
...
## Phase 2.5: 8-bit 시각 리포트 (필수)

분석 리포트 제시와 함께 `../../references/common/REPORT_8BIT.md` 가이드대로
`docs/reports/analyze-{주제}-8bit.html`을 생성하고 `open`으로 브라우저에 띄운다.
STAGE 구성 예: 요청 → 핵심 파일 카드 그리드(신뢰도 게이지) → 의존 그래프 컨베이어 →
영향 범위 HUD 테이블 → 수정 가이드 → 후속 액션.
...
```
분석 완료. 다음 중 선택하세요:
1. `/wj-magic:devrule` — 분석 결과 기반으로 바로 구현
2. `/wj-magic:plan` — 수정 태스크를 단계별로 분해
3. `/wj-magic:tdd` — 테스트부터 작성 후 구현
