...
실제 스키마·마이그레이션 코드까지 생성하는 스킬. "일단 PostgreSQL 하나"가 아니라
워크로드마다 최적 DB를 매칭하되 운영 복잡도를 저울질해 **최소 조합**을 권고한다.
...
1. **폴리글랏 우선, 최소 조합** — 워크로드를 쪼개 각각 최적 DB를 매칭하되,
   DB 하나 추가의 운영 비용(백업·모니터링·정합성)을 계상해 불필요한 다중화를 배제한다.
