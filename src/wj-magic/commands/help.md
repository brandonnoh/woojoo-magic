...
| 커맨드 | 인자 | 역할 |
|--------|------|------|
| `/wj-magic:help` | — | 커맨드·스킬 전체 목록과 사용법 안내 |
| `/wj-magic:init` | `[--with-prd]` | 프로젝트 최초 세팅 — docs/ + .dev/ + CLAUDE.md 구조 생성 |
| `/wj-magic:loop` | `plan <요구사항> \| start [task-id] \| stop \| status` | 자율 개발 루프 — PRD 생성→에이전트 구현→검증 사이클 자동 반복 |
| `/wj-magic:verify` | `[--smoke]` | 전체 빌드 + 테스트 수동 실행 — 커밋 전 최종 게이트 |
| `/wj-magic:check` | — | 코드베이스 품질 전수 점검 — 파일 크기·복잡도·any·silent catch 위반 리포트 |
| `/wj-magic:explain` | — | 코드·개념 해설 — 바이브코더 눈높이로 시스템 위치·이유·대안까지 설명 |
...
| 스킬 | 역할 |
|------|------|
| `/wj-magic:investigate` | 이슈 심층 분석 — 5개 에이전트 병렬 투입으로 근본 원인 규명 후 자동 수정 |
| `/wj-magic:devrule` | 코드 구현 — 규모(S/M/L)별 직접 작성 또는 전문 에이전트 위임 |
| `/wj-magic:tdd` | 테스트 주도 구현 — Red→Green→Refactor 사이클 강제 |
| `/wj-magic:design` | 새 UI 디자인 — 비주얼 방향 설정부터 컴포넌트 구현까지 (신규 제작) |
| `/wj-magic:polish` | 기존 UI 개선 — 완성된 화면을 진단·처방해 시각적 완성도 향상 |
| `/wj-magic:brainstorm` | 아이디어 → 스펙 문서 — 막연한 아이디어를 1:1 대화로 정제해 설계 문서 완성 |
...
5. /wj-magic:loop stop             → 중단
6. /wj-magic:verify                → 전체 빌드 최종 검증
7. /wj-magic:commit                → 커밋
```
