...
리포트 제시 후 `../references/common/REPORT_8BIT.md` 가이드대로
`docs/reports/check-{날짜}-8bit.html`을 생성하고 `open`으로 브라우저에 띄운다.
STAGE 구성 예: 언어별 요약 카드 그리드 → 위반 HUD 테이블(치명/경고/권장 게이지) →
상위 위반 파일 목록 → 다음 액션.
