...
| 기존 | 새 경로 | 변경 내용 |
|---|---|---|
| `plugins/woojoo-magic/` 전체 | `src/woojoo-magic/` | 디렉토리 이동 |
| `plugins/woojoo-magic/shared-references/` | `src/woojoo-magic/references/` | 이름 변경 |
| `.claude-plugin/marketplace.json` | `.claude-plugin/marketplace.json` | `source` 경로만 `./src/woojoo-magic`로 변경 |

### 삭제

| 대상 | 이유 |
|---|---|
| `src/woojoo-magic/templates/ralph-starter-kit/` | 외부 루프 폐기 |
| `src/woojoo-magic/commands/{brand,harness,plan,result,smoke-init,spec-init,standards}.md` | Ralph 종속 or 통합됨 |
| `src/woojoo-magic/skills/{init-prd,implement-next,feedback-to-prd,seo-optimizer,ui-ux-pro-max,senior-frontend,backend-dev-rules}/` | 프루닝 |
| `src/woojoo-magic/hooks/install-mcp.sh` | bootstrap과 중복 |
...
| 파일 | 역할 |
|---|---|
| `src/woojoo-magic/lib/gate-l1.sh` | 정적 grep 감사 (<1초) |
| `src/woojoo-magic/lib/gate-l2.sh` | tsc 증분 타입체크 (2~10초) |
| `src/woojoo-magic/lib/gate-l3.sh` | targeted test (5~30초) |
| `src/woojoo-magic/lib/journal.sh` | .dev/journal/ append |
| `src/woojoo-magic/lib/loop-state.sh` | .dev/state/loop.state 관리 |
| `src/woojoo-magic/lib/tasks-sync.sh` | tasks.json 읽기/검증/다음 task 선별 |
| `src/woojoo-magic/hooks/stop-loop.sh` | Stop hook 메인 — L1/L2/L3 + journal + 태스크 전진 |
| `src/woojoo-magic/commands/loop.md` | `/wj:loop start/stop/status` |
| `src/woojoo-magic/commands/verify.md` | `/wj:verify [--smoke]` |
| `src/woojoo-magic/templates/CLAUDE.template.md` | /wj:init용 스켈레톤 |
| `src/woojoo-magic/templates/docs/prd.template.md` | /wj:init --with-prd용 |
| `src/woojoo-magic/templates/.dev/tasks.template.json` | /wj:init용 빈 레지스트리 |
...
| 파일 | 변경 |
|---|---|
| `src/woojoo-magic/hooks/bootstrap.sh` | 176줄 → ~30줄 (자동 복사/패치/commit 전부 제거) |
| `src/woojoo-magic/hooks/hooks.json` | Stop hook 추가 |
| `src/woojoo-magic/commands/init.md` | 클린 스캐폴딩으로 전면 재작성 |
| `src/woojoo-magic/commands/help.md` | 새 커맨드 반영 |
| `src/woojoo-magic/.claude-plugin/plugin.json` | version 3.0.0, description 업데이트 |
| `src/woojoo-magic/settings.json` | 변경 없음 (유지) |
...
**Files:**
- Move: `plugins/woojoo-magic/` → `src/woojoo-magic/`
- Move: `src/woojoo-magic/shared-references/` → `src/woojoo-magic/references/`
...
```bash
git mv plugins/woojoo-magic src/woojoo-magic
```

- [ ] **Step 2: shared-references → references 이름 변경**

```bash
git mv src/woojoo-magic/shared-references src/woojoo-magic/references
```
...
**Files:**
- Delete: `src/woojoo-magic/templates/ralph-starter-kit/` (전체)

- [ ] **Step 1: ralph-starter-kit 디렉토리 삭제**

```bash
rm -rf src/woojoo-magic/templates/ralph-starter-kit
```
...
**Files:**
- Delete: `src/woojoo-magic/commands/{brand,harness,plan,result,smoke-init,spec-init,standards}.md`
- Delete: `src/woojoo-magic/skills/{init-prd,implement-next,feedback-to-prd,seo-optimizer,ui-ux-pro-max,senior-frontend,backend-dev-rules}/`
- Delete: `src/woojoo-magic/hooks/install-mcp.sh`

- [ ] **Step 1: Ralph 종속 + 레거시 커맨드 삭제**

```bash
rm src/woojoo-magic/commands/brand.md
rm src/woojoo-magic/commands/harness.md
rm src/woojoo-magic/commands/plan.md
rm src/woojoo-magic/commands/result.md
rm src/woojoo-magic/commands/smoke-init.md
rm src/woojoo-magic/commands/spec-init.md
rm src/woojoo-magic/commands/standards.md
```

- [ ] **Step 2: 프루닝 대상 스킬 삭제**

```bash
rm -rf src/woojoo-magic/skills/init-prd
rm -rf src/woojoo-magic/skills/implement-next
rm -rf src/woojoo-magic/skills/feedback-to-prd
rm -rf src/woojoo-magic/skills/seo-optimizer
rm -rf src/woojoo-magic/skills/ui-ux-pro-max
rm -rf src/woojoo-magic/skills/senior-frontend
rm -rf src/woojoo-magic/skills/backend-dev-rules
```

- [ ] **Step 3: install-mcp.sh 삭제**

```bash
rm src/woojoo-magic/hooks/install-mcp.sh
```
...
**Files:**
- Rewrite: `src/woojoo-magic/hooks/bootstrap.sh`
...
`src/woojoo-magic/hooks/bootstrap.sh`:
...
```bash
git add src/woojoo-magic/hooks/bootstrap.sh
...
**Files:**
- Create: `src/woojoo-magic/templates/CLAUDE.template.md`
- Create: `src/woojoo-magic/templates/docs/prd.template.md`
- Create: `src/woojoo-magic/templates/.dev/tasks.template.json`
- Rewrite: `src/woojoo-magic/commands/init.md`

- [ ] **Step 1: CLAUDE.template.md 작성**

`src/woojoo-magic/templates/CLAUDE.template.md`:
...
`src/woojoo-magic/templates/docs/prd.template.md`:
...
`src/woojoo-magic/templates/.dev/tasks.template.json`:
...
`src/woojoo-magic/commands/init.md`:
...
```bash
git add src/woojoo-magic/templates/ src/woojoo-magic/commands/init.md
...
```bash
chmod +x src/woojoo-magic/lib/gate-l1.sh
```
...
```bash
# 통과 케이스 — 빈 입력
echo "" | bash src/woojoo-magic/lib/gate-l1.sh
# Expected: exit 0 (no output or "[L1] OK")

# 실패 케이스 — 300줄 초과 파일이 있다면
# find . -name "*.ts" | bash src/woojoo-magic/lib/gate-l1.sh
```

- [ ] **Step 4: 커밋**

```bash
git add src/woojoo-magic/lib/gate-l1.sh
...
```bash
chmod +x src/woojoo-magic/lib/gate-l2.sh
git add src/woojoo-magic/lib/gate-l2.sh
...
```bash
chmod +x src/woojoo-magic/lib/gate-l3.sh
git add src/woojoo-magic/lib/gate-l3.sh
...
**Files:**
- Create: `src/woojoo-magic/lib/loop-state.sh`
- Create: `src/woojoo-magic/lib/journal.sh`
- Create: `src/woojoo-magic/lib/tasks-sync.sh`

### 9A: loop-state.sh

- [ ] **Step 1: loop-state.sh 작성**

`src/woojoo-magic/lib/loop-state.sh`:
...
`src/woojoo-magic/lib/journal.sh`:
...
`src/woojoo-magic/lib/tasks-sync.sh`:
...
```bash
chmod +x src/woojoo-magic/lib/loop-state.sh src/woojoo-magic/lib/journal.sh src/woojoo-magic/lib/tasks-sync.sh
git add src/woojoo-magic/lib/
...
**Files:**
- Create: `src/woojoo-magic/hooks/stop-loop.sh`
- Modify: `src/woojoo-magic/hooks/hooks.json`

Stop hook의 심장. 매 Claude 응답 종료 시 실행되어 L1→L2→L3 게이트 + journal + 태스크 전진을 수행한다.

- [ ] **Step 1: stop-loop.sh 작성**

`src/woojoo-magic/hooks/stop-loop.sh`:
...
`src/woojoo-magic/hooks/hooks.json`:
...
```bash
chmod +x src/woojoo-magic/hooks/stop-loop.sh
git add src/woojoo-magic/hooks/
...
**Files:**
- Create: `src/woojoo-magic/commands/loop.md`
- Create: `src/woojoo-magic/commands/verify.md`

- [ ] **Step 1: commands/loop.md 작성**

`src/woojoo-magic/commands/loop.md`:
...
`src/woojoo-magic/commands/verify.md`:
...
```bash
git add src/woojoo-magic/commands/loop.md src/woojoo-magic/commands/verify.md
...
**Files:**
- Rewrite: `src/woojoo-magic/commands/help.md`
- Modify: `src/woojoo-magic/.claude-plugin/plugin.json`

- [ ] **Step 1: help.md 재작성**

`src/woojoo-magic/commands/help.md`:
...
`src/woojoo-magic/.claude-plugin/plugin.json`:
...
```bash
git add src/woojoo-magic/commands/help.md src/woojoo-magic/.claude-plugin/plugin.json
...
- Verify: `src/woojoo-magic/.claude-plugin/plugin.json` (Task 12에서 이미 업데이트)
