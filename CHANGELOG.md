...
- **`ideation` 스킬 폐기 → `venture`로 통합 (스킬 20 → 19종)**: 두 스킬의 경계가 실사용에서 존재하지 않았다. 실제 조사는 `스쿼드로 전략 확정 → 심문했더니 전제가 깨짐 → 축을 다시 발굴 → 또 깨짐 → 실측으로 원본 전제 붕괴 → 다시 발굴 → 마침내 생존 아이디어 도달`으로 흘렀고, 별개 스킬이 아니라 **한 루프의 단계**였다. `ideation`은 `venture`의 **DESIGN 모드**로 흡수했다.
  - **폐기 이유**: 직군 기반 5인 스쿼드(PM/UX/사업/마케팅/데이터)가 리포트를 내면 그게 곧 결론이 됐고, 아무도 그 리포트의 전제를 확인하지 않았다. 실제 사례로 원본 기획서가 "플랫폼이 이미 흡수했으니 안 된다"며 1순위 아이템을 배제했는데 **개발자 문서를 한 번도 열어본 적이 없었고**, 나중에 워커가 curl 한 번 쳐보니 사업 방향 전체가 뒤집혔다.
  - **4개 모드 자동 판별**(사용자에게 모드를 묻지 않는다): `DISCOVER`(아이템 없음/후보 미정 → 발굴 후 실격 필터) · `DESIGN`(아이템 확정 → 다각도 병렬 리서치 → 통합 스펙·로드맵) · `INTERROGATE`(기존 판단 존재 → 실측 심문) · `SYNTHESIZE`(리포트 2개 이상 → 교차 검증). **파이프라인이 아니라 루프** — 심문이 전제를 깨면 자동으로 발굴로 회귀한다.
  - **만들기 장치 5개**(쓸 수 있는 아이디어가 나오게): `M1` 아이템 6필드 형식 강제(정의·첫 고객과 채널 실명·첫 매출까지 시간·구체 가격·실격 필터 항목별 판정·3년 낙관/비관) · `M2` 창업자의 실제 자산 주입(보유 기술/이미 만든 것/네트워크/자본/가용 시간/도메인 경험 — 없으면 교과서적 아이템만 나온다) · `M3` 탈락 후보와 이유 기록 · `M4` 반대 신호 탐색 강제 · `M5` 공통 실격 필터 선정의(비교 가능성 확보).
  - **부수기 장치 4개**(허약한 아이디어를 걸러냄): `B1` **근거 등급 표기 강제**(`[실측]`>`[문서]`>`[자체발표]`>`[추정]`>`[미검증]`) — 이게 심장이다. 실제로 *"$8B ARR"*이 다른 소스에선 $2.5B였고 *"$2B ARR"*은 실질 순매출이 3분의 1이었다. **판단을 뒤집은 것은 전부 실측이었다.** · `B2` 워커를 직군이 아니라 **검증할 명제**로 분할 · `B3` §0 판정 문장 강제(양비론 금지, 모르면 "판단 불가") · `B4` 교차 검증(독립 일치/충돌/뒤집힘 3분류, 충돌은 근거 등급으로 판정).
  - **운영 규칙 4개**(실패에서 나온 것): `O1` 파일 저장 최우선·응답은 판정 1줄+근거 3줄(저장 지시를 산출 항목 **첫 줄**에) · `O2` 웹 검색 예산 관리(코디네이터용 20회 확보) · `O3` 폐기 방향은 삭제 말고 `docs/_archive/`로 · `O4` 워커 3~5명, 초과 시 라운드 분할.
  - **종료 조건 명문화**: 승자 6필드가 전부 `[실측]`/`[문서]` 등급 + 2라운드 연속 전제 붕괴 없음 + 반대 신호를 안고도 판정 유지.
- **레퍼런스 재구성**(`skills/venture/references/`): `modes.md`(모드 판별표·루프 전이 규칙·라운드 운영) · `worker-prompts.md`(발굴용/심문용 템플릿 2종 + 공통 주입 4줄 + 조기 종료·저장 누락·예산 소진 대응) · `contracts.md`(근거 등급·판정 문장·아이템 6필드·탈락 기록·교차 검증 규약) · `design-axes.md`(DESIGN 모드 축 7종 — 구 5인 스쿼드 리서치 범위를 **직군이 아닌 명제**로 재구성) · `filters.md`(실격 필터 M5 신설 + 기존 킬러 5렌즈) · `craft.md`(감각 디자인, 근거 등급 표기 연동). 구 `squad.md`는 `worker-prompts.md`로 흡수 후 삭제.
- **트리거 키워드 병합**: `venture` frontmatter에 구 `ideation` 키워드 전부 흡수 — 아이데이션·ideation·brainstorm·기능 기획해줘·제품 전략·시장 분석·경쟁사 비교·PM/UX 관점·마케팅 전략 + 재검토·전제 확인·정말 그런가·이 판단 맞아?. **"아이데이션"이라는 단어 자체는 모드를 결정하지 않는다 — 아이템 유무가 결정한다.**
- **문서 동기화**: `commands/help.md`(스킬 표·8-bit 대상 10종→9종) · `references/common/REPORT_8BIT.md`(적용 대상에서 ideation 제거) · `README.md`(구조도·흐름도·디렉터리 트리·스킬 표, 배너 개수 17스킬/21에이전트 → 19/26 정정) · `plugin.json`·`marketplace.json`.

- **플로우차트 사이트 동기화**(`demos/flowchart/` — GitHub Pages 배포): 노드가 v4.13 시점에 멈춰 있어 실제 플러그인과 어긋나 있었다. `ideation` 노드를 `venture`로 교체하고, 누락돼 있던 `db-design`·`aeo` 스킬과 에이전트 5명(db-architect + AEO 4종)을 추가해 **51 → 58 노드**(스킬 17→19, 에이전트 21→26)로 맞췄다.
  - 엣지 신설: `plan → db-design → devrule`, `db-design → db-architect`, `commit → aeo`, `aeo → AEO 에이전트 4종`. 진입점 "아이디어 없는 경우" 설명도 스쿼드 5명 → venture 발굴·심문 루프로 교체.
  - **존 박스 정합성 교정**: 점선 벤치 박스가 노드를 가로지르던 문제를 수치로 잡았다(meta·impl/review·analyze·audit·hooks). 이제 모든 존의 `tag` 표기와 실제 포함 노드 수가 일치한다(3/9/4/8/4/7). viewBox 폭 3040 → 3360(기존에 보안 감사 마지막 노드가 28px 잘려 있었다).
  - **재발 방지**: 인트로 헤더의 `v4.13.0 · 51 NODES`와 본문 분기 수가 하드코딩돼 어긋난 것이므로, 노드 수는 `GRAPH.nodes.length`에서 직접 읽도록 바꾸고 버전만 `PLUGIN_VERSION` 상수로 남겼다. `index.html` OG/트위터 메타와 `demos/flowchart/README.md` 배지도 갱신.
- **`/wj-magic:help` 정비**: 커맨드·스킬 표 머리에 개수(8개 / 19개)를 명시하고, 워크플로에 0단계(`venture` — 만들 것이 미정일 때)를 추가했다.

### Removed

- `skills/ideation/`(SKILL.md + references/squad.md) 삭제. 기능은 `venture` DESIGN 모드로 이관됐다.
...
- **db-design 기본 철학을 "PG-first" → "워크로드-최적-우선(중립)"으로 재조정**: 4.17.0의 `db-selection-guide.md §0`이 "PostgreSQL로 시작, 무너지는 임계에서만 특화 DB 추가"로 PG쪽에 저울추가 기울어 있어, 서비스에 따라 처음부터 폴리글랏이 최적인 경우에도 PG-단독으로 편향될 소지가 있었다. **디폴트 DB를 미리 깔지 않고 워크로드를 먼저 분석해 각각에 진짜 최적인 DB를 매칭**하도록 수정. PG는 "자동 결론"이 아니라 "강력한 기본 후보 겸 임계값 검증용 기준선"으로 재정의.
- **두 실패를 대칭적으로 경계**: ① 조기 폴리글랏(추측성 다중화) ② PG-도그마(특화 DB가 명백히 필요한데 억지로 하나에 밀어넣기) — 둘 다 결함으로 명시. 운영 복잡도는 여전히 비용으로 계상하되, 성능·적합성 이득이 크면 **처음부터 폴리글랏도 권고**하도록 양방향 균형.
- 반영 파일: `skills/db-design/references/db-selection-guide.md`(§0+인트로), `skills/db-design/SKILL.md`(철학), `agents/db-architect.md`(폴리글랏 매칭 원칙 3).
...
- **신규 스킬 `/wj-magic:venture`**: 막연한 가설·타겟·관심사(또는 아무것도 없는 상태)에서 시작해 사업 아이템을 발굴·검증·폐기하고, 살아남은 승자를 사업성·UX·CX·감각 디자인까지 딥다이브해 발행 아티팩트와 프로젝트 문서로 정리하는 **0→1 벤처 파이프라인**. `ideation`(방향 정해진 뒤 심층 분석)·`brainstorm`(스펙 정제)·`plan`(태스크 분해)의 **앞단** — "무엇을 만들지 자체가 미정인 구간"을 담당한다. 6단계: **DIVERGE(후보 20~35개 확장) → GROUND(실측 데이터 스코어카드) → FILTER(킬러 5필터로 폐기) → PROVE(6축 딥다이브 + go/no-go 게이트) → CRAFT(anti-slop 감각 디자인·손끝 인터랙티브 시안) → SHIP(문서 정리)**. 각 단계 병렬 워커 fan-out으로 실측 리서치 후 발행 아티팩트 1개 산출.
- **references 3종**: `filters.md`(AI 생존성·운영 경량·진짜 사업성·경쟁 공백·벤치마크 검증 5개 킬러 필터 + 두 렌즈 교차), `squad.md`(단계별 병렬 워커 패턴 + 조기종료 재개·검색예산 우회), `craft.md`(anti-slop 원칙·레퍼런스 대비 인격 정의·손끝 감각 인터랙티브 아티팩트).

### Rationale (왜 이 스킬이 필요했나)

[2026-07] 포카라이브 프로젝트에서 "커뮤니티가 중요하다"는 막연한 가설 하나로 시작해 35개 버티컬 발굴 → 데이터 검증 → 여러 아이템(AI 도우미·은퇴 코호트·음성 자서전·시니어 케어) 정직한 폐기 → K-pop 포토카드 라이브 경매로 수렴 → 사업성·UX·CX·감각 디자인·문서화까지 이어진 실제 파이프라인을 스킬로 승격. 기존 `ideation`/`brainstorm`은 "방향이 정해진 뒤"의 도구라, "무엇을 만들지 자체가 미정"인 0→1 앞단이 비어 있었음. 핵심 차별점은 **낙관보다 반증, 아이디어보다 폐기에 무게를 두는 것**(cheerleading 금지 + AI 시대 생존 필터).
